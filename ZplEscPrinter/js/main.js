...
function isBase64(str) {
    ...
    try {
        ...
        if (!/^[A-Za-z0-9+/=]+$/.test(s) || s.length % 4 !== 0) return false;
...
async function escpos(data,b64){
    ...
    console.log('[ESC/POS]  dataAux: ', dataAux);

    if (!hasEscposCommands) {
        // No ESC/POS commands detected, print as plain text
        const html = `<html style="background-color:#f8f8f8;"><head></head><body style="background-color:white;border:1px #dee2e6 solid;"><div style="font-family:monospace;white-space:pre;">${dataAux.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</div></body></html>`;
        await displayEscPosLabel(btoa(html));
    } else if (!dataAux) { //empty
        console.log('[ESC/POS]  dataAux key 1 ');
        ...
        console.log('[ESC/POS]  dataAux key 4 ');
        console.log('[ESC/POS]  width: ', width);
        await $.post("http://localhost:8080/esc2html_service.php", {
            esc: b64 ? data : base64EncodeUnicode(dataAux), width: width
        }).done(function (response) {
            console.log('[ESC/POS] Server response:', response);
...
function startTcpServer() {
    ...
    // Do not set encoding, receive Buffer for raw data
        clientSocketInfo = {
        ...
        async function processBuffer() {
            ...
            try {
                ...
                if ($('#isZpl').is(':checked')) {
                    ...
                    if (isBase64(str)) {
                        // Data is already base64, use it directly
                        console.log('[TCP] Processing base64 encoded data, length:', str.length);
                        const response = await escpos(str, null, true);
                        if (response) sock.write(response);
                    } else {
                        // Data is raw binary ESC/POS, convert to base64 preserving binary integrity
                        console.log('[TCP] Processing raw ESC/POS data, converting to base64, length:', bufferData.length);
                        const base64Data = bufferData.toString('base64');
                        const response = await escpos(base64Data, null, true);
                        if (response) sock.write(response);
                    }
                }
